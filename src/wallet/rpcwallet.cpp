...
UniValue createcontract(const JSONRPCRequest& request){
    ...
    uint64_t blockGasLimit = agricoinDGP.getBlockGasLimit(chainActive.Height());
    uint64_t minGasPrice = CAmount(agricoinDGP.getMinGasPrice(chainActive.Height()));
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 6)
        throw runtime_error(
                ...
                "3. gasPrice  (numeric or string, optional) gasPrice AGRICOIN price per gas unit, default: "+FormatMoney(nGasPrice)+", min:"+FormatMoney(minGasPrice)+"\n"
...
UniValue sendmany(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 5)
        throw runtime_error(
            ...
            "                           Those recipients will receive less agricoins than you enter in their corresponding amount field.\n"
...
UniValue walletpassphrase(const JSONRPCRequest& request)
{
    ...
    if (pwalletMain->IsCrypted() && (request.fHelp || request.params.size() < 2 || request.params.size() > 3))
        throw runtime_error(
            ...
            "This is needed prior to performing transactions related to private keys such as sending agricoins\n"
            "\nArguments:\n"
...
UniValue lockunspent(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw runtime_error(
            ...
            "A locked transaction output will not be chosen by automatic coin selection, when spending agricoins.\n"
