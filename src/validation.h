...
#include <agricoin/agricoinDGP.h>
#include <libethereum/ChainParams.h>
#include <libethashseal/Ethash.h>
#include <libethashseal/GenesisInfo.h>
#include <script/standard.h>
#include <agricoin/storageresults.h>


extern std::unique_ptr<AgricoinState> globalState;
...
using ExtractAgricoinTX = std::pair<std::vector<AgricoinTransaction>, std::vector<EthTransactionParams>>;
///////////////////////////////////////////
...
static const size_t MAX_CONTRACT_VOUTS = 1000; // agricoin
...
void EnforceContractVoutLimit(ByteCodeExecResult& bcer, ByteCodeExecResult& bcerOut, const dev::h256& oldHashAgricoinRoot,
    const dev::h256& oldHashStateRoot, const std::vector<AgricoinTransaction>& transactions);
...
class AgricoinTxConverter{

public:

    AgricoinTxConverter(CTransaction tx, CCoinsViewCache* v = NULL, const std::vector<CTransactionRef>* blockTxs = NULL) : txBit(tx), view(v), blockTransactions(blockTxs){}

    bool extractionAgricoinTransactions(ExtractAgricoinTX& agricoinTx);
...
private:
    ...
    AgricoinTransaction createEthTX(const EthTransactionParams& etp, const uint32_t nOut);
...
public:
    ...
    ByteCodeExec(const CBlock& _block, std::vector<AgricoinTransaction> _txs, const uint64_t _blockGasLimit) : txs(_txs), block(_block), blockGasLimit(_blockGasLimit) {}
