...
bool ToHash160(const std::string& strAgricoinAddress, std::string& strHash160)
{
    CBitcoinAddress agricoinAddress(strAgricoinAddress);
    if(agricoinAddress.IsValid()){
        CKeyID keyid;
        agricoinAddress.GetKeyID(keyid);
...
bool ToAgricoinAddress(const std::string& strHash160, std::string& strAgricoinAddress)
{
    uint160 key(ParseHex(strHash160.c_str()));
    CKeyID keyid(key);
    CBitcoinAddress agricoinAddress;
    agricoinAddress.Set(keyid);
    if(agricoinAddress.IsValid()){
        strAgricoinAddress = agricoinAddress.ToString();
        return true;
    }
    return false;
}
...
bool Token::execEvents(int64_t fromBlock, int64_t toBlock, int func, std::vector<TokenEvent> &tokenEvents)
{
    ...
    for(int i = 0; i < list.size(); i++)
    {
        ...
        for(int i = 0; i < listLog.size(); i++)
        {
            ...
            ToAgricoinAddress(tokenEvent.sender, tokenEvent.sender);
            tokenEvent.receiver = topicsList[2].toString().toStdString().substr(24);
            ToAgricoinAddress(tokenEvent.receiver, tokenEvent.receiver);
