...
{
    if (pindexBestHeader->nHeight >= Params().GetConsensus().nLastPOWBlock)//toniagricoin edit
        return 0;
...
UniValue blockheaderToJSON(const CBlockIndex* blockindex)
{
    ...
    result.push_back(Pair("hashStateRoot", blockindex->hashStateRoot.GetHex())); // agricoin
    result.push_back(Pair("hashUTXORoot", blockindex->hashUTXORoot.GetHex())); // agricoin
...
UniValue blockToJSON(const CBlock& block, const CBlockIndex* blockindex, bool txDetails = false)
{
    ...
    result.push_back(Pair("hashStateRoot", block.hashStateRoot.GetHex())); // agricoin
    result.push_back(Pair("hashUTXORoot", block.hashUTXORoot.GetHex())); // agricoin
...
UniValue callcontract(const JSONRPCRequest& request)
{
    ...
    if(request.params.size() == 3){
        CBitcoinAddress agricoinSenderAddress(request.params[2].get_str());
        if(agricoinSenderAddress.IsValid()){
            CKeyID keyid;
            agricoinSenderAddress.GetKeyID(keyid);
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ ----------
    ...
    { "blockchain",         "callcontract",           &callcontract,           true,  {"address","data"} }, // agricoin
