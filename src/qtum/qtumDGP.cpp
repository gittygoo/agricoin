#include "agricoinDGP.h"

void AgricoinDGP::initDataEIP158(){
...
bool AgricoinDGP::checkLimitSchedule(const std::vector<uint32_t>& defaultData, const std::vector<uint32_t>& checkData){
...
dev::eth::EVMSchedule AgricoinDGP::getGasSchedule(unsigned int blockHeight){
    clear();
...
uint64_t AgricoinDGP::getUint64FromDGP(unsigned int blockHeight, const dev::Address& contract, std::vector<unsigned char> data){
    uint64_t value = 0;
...
uint32_t AgricoinDGP::getBlockSize(unsigned int blockHeight){
    clear();
...
uint64_t AgricoinDGP::getMinGasPrice(unsigned int blockHeight){
    clear();
...
uint64_t AgricoinDGP::getBlockGasLimit(unsigned int blockHeight){
    clear();
...
bool AgricoinDGP::initStorages(const dev::Address& addr, unsigned int blockHeight, std::vector<unsigned char> data){
    initStorageDGP(addr);
...
void AgricoinDGP::initStorageDGP(const dev::Address& addr){
    storageDGP = state->storage(addr);
}

void AgricoinDGP::initStorageTemplate(const dev::Address& addr){
    storageTemplate = state->storage(addr);
}

void AgricoinDGP::initDataTemplate(const dev::Address& addr, std::vector<unsigned char>& data){
    dataTemplate = CallContract(addr, data)[0].execRes.output;
}

void AgricoinDGP::createParamsInstance(){
...
dev::Address AgricoinDGP::getAddressForBlock(unsigned int blockHeight){
...
void AgricoinDGP::parseStorageScheduleContract(std::vector<uint32_t>& uint32Values){
...
void AgricoinDGP::parseDataScheduleContract(std::vector<uint32_t>& uint32Values){
...
void AgricoinDGP::parseStorageOneUint64(uint64_t& value){
...
void AgricoinDGP::parseDataOneUint64(uint64_t& value){
    if(dataTemplate.size() == 32){
        value = uint64_t(dev::u256(dev::h256(dataTemplate)));
    }
}

dev::eth::EVMSchedule AgricoinDGP::createEVMSchedule(){
    dev::eth::EVMSchedule schedule = dev::eth::EIP158Schedule;
