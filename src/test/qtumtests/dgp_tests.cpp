...
void contractLoading(){
    dev::eth::ChainParams cp((dev::eth::genesisInfo(dev::eth::Network::agricoinMainNetwork)));
...
void createTestContractsAndBlocks(TestChain100Setup* testChain100Setup, valtype& code1, valtype& code2, valtype& code3, dev::Address addr){
    ...
    txs.push_back(createAgricoinTransaction(code[0], 0, dev::u256(500000), dev::u256(1), hashTemp, addr, 0));
    txs.push_back(createAgricoinTransaction(code1, 0, dev::u256(500000), dev::u256(1), ++hashTemp, dev::Address(), 0));
    txs.push_back(createAgricoinTransaction(code[2], 0, dev::u256(500000), dev::u256(1), ++hashTemp, addr, 0));
    auto result = executeBC(txs);

    generateBlocks(50);
    txs.clear();
    txs.push_back(createAgricoinTransaction(code2, 0, dev::u256(500000), dev::u256(1), ++hashTemp, dev::Address(), 0));
    txs.push_back(createAgricoinTransaction(code[4], 0, dev::u256(500000), dev::u256(1), ++hashTemp, addr, 0));
    result = executeBC(txs);

    generateBlocks(50);
    txs.clear();
    txs.push_back(createAgricoinTransaction(code3, 0, dev::u256(500000), dev::u256(1), ++hashTemp, dev::Address(), 0));
    txs.push_back(createAgricoinTransaction(code[6], 0, dev::u256(500000), dev::u256(1), ++hashTemp, addr, 0));
    result = executeBC(txs);
}
...
BOOST_AUTO_TEST_CASE(gas_schedule_default_state_test1){
    initState();
    contractLoading();
    AgricoinDGP agricoinDGP(globalState.get());
    dev::eth::EVMSchedule schedule = agricoinDGP.getGasSchedule(100);
...
BOOST_AUTO_TEST_CASE(gas_schedule_default_state_test2){
    ...
    contractLoading();
    AgricoinDGP agricoinDGP(globalState.get());
    dev::eth::EVMSchedule schedule = agricoinDGP.getGasSchedule(0);
    BOOST_CHECK(compareEVMSchedule(schedule, dev::eth::EIP158Schedule));
}
...
BOOST_AUTO_TEST_CASE(gas_schedule_one_paramsInstance_introductory_block_1_test1){
    ...
    dev::h256 hashTemp(hash);
    std::vector<AgricoinTransaction> txs;
    txs.push_back(createAgricoinTransaction(code[0], 0, dev::u256(500000), dev::u256(1), hashTemp, GasScheduleDGP, 0));
    txs.push_back(createAgricoinTransaction(code[1], 0, dev::u256(500000), dev::u256(1), ++hashTemp, dev::Address(), 0));
    txs.push_back(createAgricoinTransaction(code[2], 0, dev::u256(500000), dev::u256(1), ++hashTemp, GasScheduleDGP, 0));
    auto result = executeBC(txs);
...
BOOST_AUTO_TEST_CASE(gas_schedule_one_paramsInstance_introductory_block_1_test2){
    ...
    AgricoinDGP agricoinDGP(globalState.get());
    dev::eth::EVMSchedule schedule = agricoinDGP.getGasSchedule(502); // After initializing the tests, the height of the chain 502
...
BOOST_AUTO_TEST_CASE(gas_schedule_passage_from_0_to_130_three_paramsInstance_test){
    ...
    for(size_t i = 0; i < 1300; i++){
        dev::eth::EVMSchedule schedule = agricoinDGP.getGasSchedule(i);
...
BOOST_AUTO_TEST_CASE(block_size_default_state_test1){
    ...
    contractLoading();
    AgricoinDGP agricoinDGP(globalState.get());
    uint32_t blockSize = agricoinDGP.getBlockSize(100);
...
BOOST_AUTO_TEST_CASE(block_size_default_state_test2){
    ...
    contractLoading();
    AgricoinDGP agricoinDGP(globalState.get());
    uint32_t blockSize = agricoinDGP.getBlockSize(0);
    BOOST_CHECK(blockSize == DEFAULT_BLOCK_SIZE_DGP);
}
...
BOOST_AUTO_TEST_CASE(block_size_one_paramsInstance_introductory_block_1_test1){
    ...
    dev::h256 hashTemp(hash);
    std::vector<AgricoinTransaction> txs;
    txs.push_back(createAgricoinTransaction(code[0], 0, dev::u256(500000), dev::u256(1), hashTemp, BlockSizeDGP, 0));
    txs.push_back(createAgricoinTransaction(code[7], 0, dev::u256(500000), dev::u256(1), ++hashTemp, dev::Address(), 0));
    txs.push_back(createAgricoinTransaction(code[2], 0, dev::u256(500000), dev::u256(1), ++hashTemp, BlockSizeDGP, 0));
    auto result = executeBC(txs);
...
BOOST_AUTO_TEST_CASE(block_size_one_paramsInstance_introductory_block_1_test2){
    ...
    AgricoinDGP agricoinDGP(globalState.get());
    uint32_t blockSize = agricoinDGP.getBlockSize(502);
...
BOOST_AUTO_TEST_CASE(block_size_passage_from_0_to_130_three_paramsInstance_test){
    ...
    AgricoinDGP agricoinDGP(globalState.get());
    for(size_t i = 0; i < 1300; i++){
        uint32_t blockSize = agricoinDGP.getBlockSize(i);
...
BOOST_AUTO_TEST_CASE(min_gas_price_default_state_test1){
    ...
    contractLoading();
    AgricoinDGP agricoinDGP(globalState.get());
    uint64_t minGasPrice = agricoinDGP.getMinGasPrice(100);
...
BOOST_AUTO_TEST_CASE(min_gas_price_default_state_test2){
    ...
    contractLoading();
    AgricoinDGP agricoinDGP(globalState.get());
    uint64_t minGasPrice = agricoinDGP.getMinGasPrice(0);
    BOOST_CHECK(minGasPrice == DEFAULT_MIN_GAS_PRICE_DGP);
}
...
BOOST_AUTO_TEST_CASE(min_gas_price_one_paramsInstance_introductory_block_1_test1){
    ...
    dev::h256 hashTemp(hash);
    std::vector<AgricoinTransaction> txs;
    txs.push_back(createAgricoinTransaction(code[0], 0, dev::u256(500000), dev::u256(1), hashTemp, GasPriceDGP, 0));
    txs.push_back(createAgricoinTransaction(code[10], 0, dev::u256(500000), dev::u256(1), ++hashTemp, dev::Address(), 0));
    txs.push_back(createAgricoinTransaction(code[2], 0, dev::u256(500000), dev::u256(1), ++hashTemp, GasPriceDGP, 0));
    auto result = executeBC(txs);
...
BOOST_AUTO_TEST_CASE(min_gas_price_one_paramsInstance_introductory_block_1_test2){
    ...
    AgricoinDGP agricoinDGP(globalState.get());
    uint64_t minGasPrice = agricoinDGP.getMinGasPrice(502);
...
BOOST_AUTO_TEST_CASE(min_gas_price_passage_from_0_to_130_three_paramsInstance_test){
    ...
    AgricoinDGP agricoinDGP(globalState.get());
    for(size_t i = 0; i < 1300; i++){
        uint64_t minGasPrice = agricoinDGP.getMinGasPrice(i);
        std::function<bool(const uint64_t&, const uint64_t&)> func = compareUint64;
