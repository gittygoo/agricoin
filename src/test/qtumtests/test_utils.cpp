...
void initState(){
    ...
    const std::string dirAgricoin = pathTemp.string();
    const dev::h256 hashDB(dev::sha3(dev::rlp("")));
    globalState = std::unique_ptr<AgricoinState>(new AgricoinState(dev::u256(0), AgricoinState::openDB(dirAgricoin, hashDB, dev::WithExisting::Trust), dirAgricoin + "/agricoinDB", dev::eth::BaseState::Empty));
...
AgricoinTransaction createAgricoinTransaction(valtype data, dev::u256 value, dev::u256 gasLimit, dev::u256 gasPrice, dev::h256 hashTransaction, dev::Address recipient, int32_t nvout){
    AgricoinTransaction txEth;
    if(recipient == dev::Address()){
        txEth = AgricoinTransaction(value, gasPrice, gasLimit, data, dev::u256(0));
    } else {
        txEth = AgricoinTransaction(value, gasPrice, gasLimit, recipient, data, dev::u256(0));
    }
...
std::pair<std::vector<ResultExecute>, ByteCodeExecResult> executeBC(std::vector<AgricoinTransaction> txs){
