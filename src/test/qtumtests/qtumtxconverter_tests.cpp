...
void checkResult(bool isCreation, std::vector<AgricoinTransaction> results, uint256 hash){
...
void runTest(bool isCreation, size_t n, CScript& script1, CScript script2 = CScript()){
    ...
    AgricoinTxConverter converter(transaction, NULL);
    ExtractAgricoinTX agricoinTx;
    BOOST_CHECK(converter.extractionAgricoinTransactions(agricoinTx));
    std::vector<AgricoinTransaction> result = agricoinTx.first;
    if(script2 == CScript()){
...
BOOST_FIXTURE_TEST_SUITE(agricointxconverter_tests, TestingSetup)
