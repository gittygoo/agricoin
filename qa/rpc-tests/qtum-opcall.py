...
class OpCallTest(BitcoinTestFramework):
    ...
    def send_multiple_op_call_txs_with_counter_check(self, num_txs, outputs, counter_should_increase_by):
        ...
        while i < num_txs and len(unspents) > 0:
            # Select as input a tx which has at least 5 agricoin spendable
    ...
    def many_calls_in_same_tx_test(self):
        outputs = []
        outputs.append(make_op_call_output(0, b"\x04", CScriptNum(1000000), CScriptNum(AGRICOIN_MIN_GAS_PRICE), bytes.fromhex("371303c0"), bytes.fromhex(self.contract_address)))
        outputs.append(make_op_call_output(0, b"\x04", CScriptNum(1000000), CScriptNum(AGRICOIN_MIN_GAS_PRICE), bytes.fromhex("371303c0"), bytes.fromhex(self.contract_address)))
    ...
    def normal_op_call_output_test(self):
        outputs = []
        outputs.append(make_op_call_output(0, b"\x04", b"\xff\x7f", CScriptNum(AGRICOIN_MIN_GAS_PRICE), bytes.fromhex("371303c0"), bytes.fromhex(self.contract_address)))
    ...
    def gas_equal_to_tx_fee_test(self):
        outputs = []
        outputs.append(make_op_call_output(0, b"\x04", CScriptNum(1000000), CScriptNum(AGRICOIN_MIN_GAS_PRICE), bytes.fromhex("371303c0"), bytes.fromhex(self.contract_address)))
        self.send_one_op_call_tx_with_counter_check(outputs, counter_should_increase_by=1, input_value=1000000*AGRICOIN_MIN_GAS_PRICE)

    # Sends a tx containing 1 op_call output where txfee < gas_price*gas_limit.
    def gas_exceeding_tx_fee_100001_1_test(self):
        outputs = []
        outputs.append(make_op_call_output(0, b"\x04", CScriptNum(10000001), CScriptNum(AGRICOIN_MIN_GAS_PRICE), bytes.fromhex("371303c0"), bytes.fromhex(self.contract_address)))
        self.send_one_op_call_tx_with_counter_check(outputs, input_value=1000001*AGRICOIN_MIN_GAS_PRICE-1, should_throw=True)

    # Sends a tx containing 1 op_call output where txfee < gas_price*gas_limit.
    def gas_exceeding_tx_fee_100001_2_test(self):
        outputs = []
        outputs.append(make_op_call_output(0, b"\x04", CScriptNum(1000001), CScriptNum(AGRICOIN_MIN_GAS_PRICE), bytes.fromhex("371303c0"), bytes.fromhex(self.contract_address)))
        self.send_one_op_call_tx_with_counter_check(outputs, input_value=1000000*AGRICOIN_MIN_GAS_PRICE, should_throw=True)
    ...
    def two_calls_in_same_tx_exceeding_tx_fee_test(self):
        ...
        outputs.append(make_op_call_output(0, b"\x04", CScriptNum(1000000), CScriptNum(AGRICOIN_MIN_GAS_PRICE), bytes.fromhex("371303c0"), bytes.fromhex(self.contract_address)))
        outputs.append(make_op_call_output(0, b"\x04", CScriptNum(1000000), CScriptNum(AGRICOIN_MIN_GAS_PRICE), bytes.fromhex("371303c0"), bytes.fromhex(self.contract_address)))
        self.send_one_op_call_tx_with_counter_check(outputs, input_value=2000000*AGRICOIN_MIN_GAS_PRICE-1, should_throw=True)
    ...
    def gas_limit_signedness_test(self):
        ...
        while len(gas_limit) < 20:
            outputs.append(make_op_call_output(0, b"\x04", gas_limit, CScriptNum(AGRICOIN_MIN_GAS_PRICE), bytes.fromhex("371303c0"), bytes.fromhex(self.contract_address)))
    ...
    def gas_limit_signedness_one_valid_test(self):
        outputs = []
        gas_limit = b"\xff"
        outputs.append(make_op_call_output(0, b"\x04", b"\xff\xff\x00", CScriptNum(AGRICOIN_MIN_GAS_PRICE), bytes.fromhex("371303c0"), bytes.fromhex(self.contract_address)))
        outputs.append(make_op_call_output(0, b"\x04", b"\xff\xff", CScriptNum(AGRICOIN_MIN_GAS_PRICE), bytes.fromhex("371303c0"), bytes.fromhex(self.contract_address)))
        self.send_one_op_call_tx_with_counter_check(outputs, should_throw=True)
    ...
    def send_tx_with_value_test(self):
        outputs = []
        # d0e30db0 deposit()
        outputs.append(make_op_call_output(100000000, b"\x04", CScriptNum(1000000), CScriptNum(AGRICOIN_MIN_GAS_PRICE), bytes.fromhex("d0e30db0"), bytes.fromhex(self.contract_address)))
