...
*Setup instructions for a Gitian build of Agricoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Agricoin
...
- [Building Agricoin Core](#building-agricoin-core)
...
Clone the git repositories for agricoin and Gitian.
...
```bash
sudo cp /home/debian/agricoin/contrib/gitian-descriptors/xenial /usr/share/debootstrap/scripts
```
...
in the agricoin repository under 'Fetch and create inputs' to install sources which require
...
To build Agricoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the agricoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/agricoin/.git/
...
For example:
```bash
URL=https://github.com/laanwj/agricoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit agricoin=${COMMIT} --url agricoin=${URL} ../agricoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit agricoin=${COMMIT} --url agricoin=${URL} ../agricoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit agricoin=${COMMIT} --url agricoin=${URL} ../agricoin/contrib/gitian-descriptors/gitian-osx.yml
# if wanting to use a different version of eth-cpp-agricoin:
./bin/gbuild --commit agricoin=${COMMIT},cpp-eth-agricoin=${ETHCOMMIT} --url agricoin=${URL},cpp-eth-agricoin=${ETHURL} ../agricoin/contrib/gitian-descriptors/gitian-linux.yml
```
...
and the agricoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../agricoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/agricoin
SIGPATH=/some/root/path/agricoin-detached-sigs

./bin/gbuild --url agricoin=${BTCPATH},signature=${SIGPATH} ../agricoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/agricoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/agricoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/agricoin-osx-build.assert
```
...
[agricoinproject/gitian.sigs](https://github.com/agricoinproject/gitian.sigs/) repository, or if that's not possible create a pull
request. You can also mail the files to Jordan Earls (earlz@agricoin.org) and he will commit them.
